...
AZURE_SPEECH_KEY = "u5Kl1FgBq8JgfFA6KwWGwnxccWwO22B6cqnyYDdIPwlmgxCA6hdeJQQJ99BFACREanaXJ3w3AAAYACOGus50"
...
<script>
    ...
    async function getSpeechConfig() {{
        const res = await fetch("http://localhost:5000/api/token"); // Change if deployed
    ...
    async function assessPronunciation(referenceText) {{
        ...
        try {{
            ...
            document.getElementById("pronunciationResult").textContent = "❌ Could not start recognition.";
        }}
    }}
...
def open_browser():
    ...
    import time
    time.sleep(1)
    print("✅ Simulated browser visit to /")
    with app.test_client() as client:
        response = client.get("/")
        print("✅ / returned:", response.status_code)
        print(response.data[:100])  # preview response
