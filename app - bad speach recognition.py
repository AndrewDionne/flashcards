...
<script>
    ...
    updateCard();
    function assessPronunciation(phrase) {{
        const resultDiv = document.getElementById("pronunciationResult");
        resultDiv.textContent = "🎤 Please pronounce: " + phrase;
        const recognition = new (window.SpeechRecognition || window.webkitSpeechRecognition)();
        recognition.lang = "pl-PL"; // Set to Polish
        recognition.interimResults = false;
        recognition.onresult = (event) => {{
            const spokenPhrase = event.results[0][0].transcript.trim();
            if (spokenPhrase.toLowerCase() === phrase.toLowerCase()) {{ 
                resultDiv.textContent = "✅ Correct pronunciation!";
            }} else {{
                resultDiv.textContent = "❌ Incorrect. You said: " + spokenPhrase;
            }}
        }};
